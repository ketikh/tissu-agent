...
async def _gemini_visual_compare(user_image: bytes, candidate_urls: list[str]) -> str | None:
    ...
    try:
        ...
        parts.append(types.Part(text=(
            "TASK: Match the customer's laptop-bag photo to one of the numbered candidate reference photos. "
            "Some products have multiple reference photos (front + back) — treat each numbered reference as "
            "a separate candidate that still represents the same product.\n\n"
            "DECIDING FRAMEWORK:\n"
            "• Same product = same fabric pattern + same dominant colors + same closure style.\n"
            "• Real photos of the SAME bag can still differ by: lighting, white balance, camera filter, "
            "  crop, aspect ratio, compression artifacts, screenshot overlays, background, props "
            "  (flowers, surface, hands). Mentally correct for these before judging.\n"
            "• A shape-twin in the WRONG colour is NOT a match (orange ≠ yellow; navy ≠ royal; mint ≠ teal).\n"
            "• A different PATTERN family is NOT a match (floral vs striped vs solid).\n"
            "• Small hue shifts from lighting/filter are fine — judge the fabric colour, not the pixel hue.\n\n"
            "HANDLING DIFFICULT CUSTOMER PHOTOS:\n"
            "• Cropped social-media screenshots: the bag may fill the frame without the staged background.\n"
            "• Different angle: the bag may be open, folded, or rotated. Look at the pattern motifs, not the silhouette.\n"
            "• Low resolution: trust the overall colour/pattern impression rather than fine detail.\n\n"
            "OUTPUT:\n"
            "If any candidate is the same product (allowing for the noise above), return its number. "
            "If NONE plausibly match, return 0. Prefer returning a number when the match is likely; "
            "'no match' is reserved for cases where the product family is clearly different (different "
            "colour family or different pattern family).\n\n"
